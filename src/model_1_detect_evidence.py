...
class DataTrainingArguments:
    ...
    min_prediction_per_doc: Optional[int] = field(
        default=-1,
        metadata={
            "help": (
                "At prediction, we predict top 'min_prediction_per_doc' pages for each document_id. "
                "If the value is -1 or 0, this will not be applied."
            )
        },
    )
...
def main():
        ...
        for data_split in ['train', 'test']:
            ...
            for jd in jds:
                ...
                for page_idx in page_indices:
                    ...
                    if data_split == 'train':
                        label = 'evidence' if page_has_evidence else 'none'
                    else:
                        label = 'none'
    ...
    if training_args.do_predict:
        ...
        ).numpy()[:, label_list.index('evidence')].tolist()
            ...
            with open(output_predict_file, "w") as writer:
                ...
                for index, score in enumerate(evidence_scores):
                    ...
                    document_id_2_candidates[document_id].append({
                        "query": 'energy_transition_&_zero_carbon_technologies',  # dummy label
                ...
                for document_id, output_candidates in document_id_2_candidates.items():
                    ...
                    if data_args.min_prediction_per_doc > 0 and len(outputs) < data_args.min_prediction_per_doc:
                        output_candidates = sorted(
                            output_candidates,
                            key=lambda x: x['evidence_prob'],
                            reverse=True
                        )
                        outputs = output_candidates[:data_args.min_prediction_per_doc]
